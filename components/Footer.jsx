...
export default function Footer() {
  return (
    <footer className="schematic mt-24 bg-navy text-white">
      <div className="mx-auto max-w-shell px-6 py-16">
              ...
              {groups.map((g) => (
                <li key={g.id}>
                  <Link
                    ...
                    className="group flex items-baseline justify-between gap-3 text-[14px] text-white/70 hover:text-tint"
                  >
                    <span>{g.name}</span>
                    <span className="data text-[11px] text-white/70">{g.count}</span>
                  </Link>
                </li>
              ))}
            </ul>
          ...
          <div>
            ...
            <ul className="mt-4 space-y-3 text-[14px] text-white/70">
              ...
              <li>
                ...
                <span className="ml-2 text-[12px] text-white/70">landline</span>
              </li>
              <li>
                <a href={`https://wa.me/${COMPANY.whatsapp.replace(/\D/g, "")}`} className="data hover:text-tint">
                  {COMPANY.whatsapp}
                </a>
                <span className="ml-2 text-[12px] text-white/70">WhatsApp</span>
              </li>
              <li>
              ...
              <li className="text-white/75">{COMPANY.hours}</li>
            </ul>
          </div>

          <div>
            <p className="eyebrow text-tint/70">Delivery to vessel</p>
            <ul className="data mt-4 space-y-2 text-[13px] text-white/70">
              {COMPANY.ports.map((p) => (
                <li key={p} className="flex items-center gap-2">
                  <span className="h-1 w-1 bg-coral" />
                  {p}
                </li>
              ))}
            </ul>
        ...
        <div className="mt-14 flex flex-col gap-4 border-t border-white/12 pt-6 md:flex-row md:items-center md:justify-between">
          <p className="data text-[11px] text-white/70">
